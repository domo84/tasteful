...
use Sunnyexample\Database\SQLite;

class ExampleItem extends SQLite
{
    ...
    {
        $statement = $this->prepare("SELECT * FROM example_item WHERE id = :id");
    ...
    {
        $statement = $this->prepare("UPDATE example_item SET name = :name WHERE id = :id");
        $statement->bindValue(":id", $id);
    ...
    {
        $statement = $this->prepare("DELETE FROM example_item WHERE id = :id");
        $statement->bindValue(":id", $id);
        $statement->execute();
    }
}
